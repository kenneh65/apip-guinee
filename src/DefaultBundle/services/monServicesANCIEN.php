    ...
    { 
        ...
        try {
            ...
            if ($this->pingIPServer() == true) {
               ...
               // var_dump('is conect');die();
                $this->get('session')->getFlashBag()->add('echecMail', 'Dossier enregistré avec succès mais Mail non envoyé au promoteur un probleme lie à la connexion internet du serveur');
            }
        }
        catch (\Exception $ex){
           //  var_dump($this->check_internet_connection());die();
    ...
    public function SmsOrange($phoneNumber, Representant $representant, $typeSms = 'depot')
    {
        ...
        if (!empty($response['access_token'])) {
            ...
            if ($this->pingIPServer() == true) {
                ...
                $this->get('session')->getFlashBag()->add('echecSMS', 'Dossier enregistré avec succès mais SMS non envoyé au promoteur');
            }
            //   var_dump('ok');die();
        } else {
            $this->get('session')->getFlashBag()->add('echecSMS', 'Dossier enregistré avec succès mais SMS non envoyé au promoteur');
...
//$response = $osms->getAdminPurchasedBundles('CIV');
    ...
    {
        return (bool)@fsockopen('ssl0.ovh.net', 80, $iErrno, $sErrStr, 2);
    }
