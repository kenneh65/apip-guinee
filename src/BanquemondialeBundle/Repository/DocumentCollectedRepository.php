...
class DocumentCollectedRepository extends EntityRepository
{
    ...
    public function findDossierPole($user, $data, $idLangue, $idPole, $limit = null, $statut = null)
    {
        ...
        if ($statut && $statut == 2) {
            if ( !empty($data['dateDelivranceDebut'])) {
            ...
            if ( !empty($data['dateDelivranceFin'])) {
...
//        }
    ...
    public function findDossierPoleRetrait($user, $data, $idLangue, $idPole, $limit = null, $statut = null, $idS = null)
    {
        ...
        $sqlQuery = "SELECT dc.id as idDocumentCollected,dd.id, dc.dateDelivrance as dateReception, (select concat(r.prenom,' ',r.nom) from representant r where r.idDossierDemande = dd.id order by id ASC limit 1) as gerant , u.entreprise_id as idE,
		...
		join pole p on dc.idPole = p.id left join fos_user u on dd.idUtilisateur=u.id
    ...
    public function findDossierEnAttenteImmatriculationDGAWithParam($data, $idLangue, $idPole, $limit = null, $statut = null)
    {
      ...
      //  die(dump($results));
        $tabResultemp = [];
        foreach ($results as $result) {
            $tabResult['numeroDossier'] = $result->getDossierDemande()->getNumeroDossier();
            $tabResult['denominationSociale'] = $result->getDossierDemande()->getDenominationSociale();
            $tabResult['nomCommercial'] = $result->getDossierDemande()->getNomCommercial();
            $tabResult['enseigne'] = $result->getDossierDemande()->getEnseigne();
            $tabResult['telephone'] = $result->getDossierDemande()->getTelephone();
            $tabResult['email'] = $result->getDossierDemande()->getEmail();
            $tabResult['region'] = $result->getDossierDemande()->getRegion();
            $tabResult['dateCreation'] = $result->getDossierDemande()->getDateCreation();
            $tabResult['orange'] = $result->isOrangePole();
            $tabResult['red'] = $result->isRedPole();
            $tabResult['dateSoumission'] = $result->getDateSoumission();
            $tabResult['dateDelivrance'] = $result->getDateDelivrance();
            $tabResult['duree'] = $result->getDuree();
            $tabResult['motif'] = $result->getMotif();
            $tabResult['id'] = $result->getDossierDemande()->getId();

            $formJ = $result->getDossierDemande()->getFormeJuridique();
            $idf = $formJ->getId();
            $tabResult['idFormeJ'] = $idf;
            $formJTrad = $this->getEntityManager()->getRepository('BanquemondialeBundle:FormeJuridiqueTraduction')->getLibelleFormeJuridiqueByLanque($idLangue, $idf);
            ($formJTrad) ? $tabResult['libelleFormeJ'] = $formJTrad->getLibelle() : $tabResult['libelleFormeJ'] = "";
            $idTyp = $result->getDossierDemande()->getTypeOperation()->getId();
            $tabResult['idTypeOp'] = $idTyp;
            $typDossier = $result->getDossierDemande()->getTypeDossier()->getLibelle();
            $tabResult['typDossier'] = $typDossier;
            $typeOpTrad = $this->getEntityManager()->getRepository('BanquemondialeBundle:TypeOperationTraduction')->getLibelleOperationByLanque($idLangue, $idTyp);
            ($typeOpTrad) ? $tabResult['libelleTypeOp'] = $typeOpTrad->getLibelle() : $tabResult['libelleTypeOp'] = "";

            $statutTraitement = $result->getStatutTraitement();
            $tabResult['idStatutTraitement'] = $statutTraitement->getId();
            $statutTraitementTraduction = $this->getEntityManager()->getRepository('BanquemondialeBundle:StatutTraitementTraduction')->getLibelleStatutTraitementByLangue($idLangue, $statutTraitement);
            ($statutTraitementTraduction) ? $tabResult['libelleStatutTraitement'] = $statutTraitementTraduction->getLibelle() : $tabResult['libelleStatutTraitement'] = "";
            $formJGerant = $this->getEntityManager()->getRepository('BanquemondialeBundle:Representant')->findOneByDossierDemande($result->getDossierDemande());
            $tabResult['gerant'] = ($formJGerant) ? $formJGerant->getPrenom() . ' ' . $formJGerant->getNom() : "";
            $verifDoss = true;
            $UserSaisi = $result->getDossierDemande()->getUtilisateur();
            $UserDepot = $result->getDossierDemande()->getUtilisateurDepot();
            $tabResult['agentSaisi'] = ($UserSaisi) ? $UserSaisi->getPrenom() . ' ' . $UserSaisi->getNom() : "";
            $tabResult['agentDepot'] = ($UserDepot) ? $UserDepot->getPrenom() . ' ' . $UserDepot->getNom() : $UserSaisi->getPrenom() . ' ' . $UserSaisi->getNom();
            $entreprise = ($UserSaisi) ? $UserSaisi->getEntreprise() : "";
            $tabResult['structure'] = ($UserSaisi && $entreprise != "") ? $UserSaisi->getEntreprise()->getDenomination() : "";
            $em = $this->getEntityManager();
            $numeroNif = "";
            $numeroAguipe = "";
            $numeroCnss = "";
            ...
            $tabResult['numeroRccm'] = $numeroRccm;
            $tabResult['numeroNif'] = $numeroNif;
            $tabResult['numeroAguipe'] = $numeroAguipe;
            $tabResult['numeroCnss'] = $numeroCnss;
            array_push($tabResultemp,$tabResult);
        }

      //  die(dump(($tabResultemp)));
        return $tabResultemp;
    }
...
//partie Ajout structure
            ...
            $tabResult[$i]['typeDossier'] = $result['typeDossier'];
            if ($formulaireRccm) {
